...
class TestWebAppPreferences(unittest.TestCase):
    ...
    def test_project_controls_and_deferred_patches_are_wired(self) -> None:
        ...
        native_init_index = javascript.index("if (isNativeApp) setFullscreenLayout(true);")
        token_check_index = javascript.index("if (isTokenRequired && !token)")
        self.assertLess(native_init_index, token_check_index)
