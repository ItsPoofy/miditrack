  ...
  .action((input, options) => {
    try {
      ...
      if (options.probe) {
        ...
        const converter = new MidiConverter(playback.data, { autoTempo: true });
        converter.convert();
        const detected = Math.round(detectTempoFromOnsets(converter.onsets, converter.sampleRate));
      ...
      if (options.noiseWav !== undefined) {
        const result = renderNoiseWav(playback.data, playback.totalSamples, options.noiseWav, converter.startSampleOffset);
      ...
      if (options.dacWav !== undefined) {
        const result = renderDacWav(playback.data, playback.totalSamples, options.dacWav, converter.startSampleOffset);
        if (result.voicesFound > 0) {
