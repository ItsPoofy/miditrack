...
import { detectTempoFromOnsets } from './tempo-detect';
import {
  PCMTrackEvent,
...
export const YM2151_FM_PITCH_BEND_RANGE = 2;
export const YM2203_FM_PITCH_BEND_RANGE = 2;
export const YM2608_FM_PITCH_BEND_RANGE = 2;
export const OPL_FM_PITCH_BEND_RANGE = 2;
export const CHIP_PITCH_BEND_RANGE = 2;
...
export interface ChannelState {
  ...
  initialTuningOffset?: number; // Offset subtracted from future pitch bends when preserveChipTuning is false
...
export class MidiConverter {
  ...
  samplesToTicks(samples: number): number {
    ...
    return samplesToTicks(adjusted, this.internalTempo, this.sampleRate);
  }
  ...
  calculateStartSampleOffset(): number {
    if (this.onsets.length === 0) return 0;
    const firstOnset = Math.min(...this.onsets);
    if (firstOnset <= 0) return 0;
    ...
    for (let i = 0; i < this.vgmData.commands.length; i++) {
      ...
      if (cmd.type === 'wait' && cmd.samples) {
        ...
        this.withChipInstance(cmd.chip ?? 'unknown', cmd.instance ?? 0, () => {
          ...
          else if (cmd.chip === 'YM2151') handleYM2151Write(this, cmd, currentTime, activeNotes, i);
  ...
  private csmPulseSamples(): number {
    return Math.max(1, (CSM_MIDI_PULSE_TICKS * 60 * this.sampleRate) / (this.internalTempo * MIDI_PPQ));
  }
  ...
  updateKeyBoundFMPitch(
    ...
    if (!activeNotes.has(key)) {
      if (state.active) noteOn(this, key, midiChannelOffset, currentTime, activeNotes);
      return;
    }

    const frequency = getNoteFrequency(this, key, state);
    if (frequency <= 20) return;
    const exactMidi = frequencyToExactMidi(frequency);
  ...
  peekUpcomingYM2151KeyCode(cmdIndex: number, channel: number, instance = 0): void {
    const key = `ym2151_${channel}`;
    const state = this.channels.get(key);
    if (!state) return;
    ...
    for (let index = cmdIndex + 1; index < this.vgmData.commands.length; index++) {
      ...
      if (foundCode && foundFraction) break;
    }
  }

  peekUpcomingSSGPeriod(
    cmdIndex: number,
    chip: string,
    channel: number,
    instance = 0,
    ...
    for (let index = cmdIndex + 1; index < this.vgmData.commands.length; index++) {
      ...
      if ((next.port ?? 0) !== 0) continue;

      if (next.register === 8 + channel && (next.data ?? 0) === 0) break;
      if (next.register === 7 && ((next.data ?? 0) & (1 << channel)) !== 0) break;
  ...
  exportToFile(outputPath: string): void {
    ...
    if (this.options.autoTempo) {
      const detected = detectTempoFromOnsets(this.onsets, this.sampleRate);
