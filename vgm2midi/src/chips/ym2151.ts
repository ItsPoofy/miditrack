...
import { YM2151_FM_PITCH_BEND_RANGE } from '../midi-converter';
import { noiseDrumNote } from '../midi-math';
import { addPan, addExpression, noteOn, noteOff, noteOnPercussion } from '../event-output';
...
export function updateOPMCsmTimerRegister(host: MidiConverter, instance: number, register: number, data: number): void {
...
export function updateOPMCsmTimer(
  host: MidiConverter,
  instance: number,
  data: number,
  currentTime: number,
  ...
  if (!isActive) {
    if (timer.nextRelease !== undefined) host.emitOPMCsmPulse(instance, false, currentTime, activeNotes);
...
export function handleYM2151Write(
  host: MidiConverter,
  cmd: VGMCommand,
  currentTime: number,
  activeNotes: ActiveNoteMap,
  ...
  if (reg >= 0x60 && reg <= 0x7F) {
    ...
    if (state.active) {
      addExpression(host, `ym2151_${channel}`, host.opnCarrierExpression(state), currentTime);
    }
  ...
  if (reg === 0x0F) {
    ...
    if (
      ...
      noteOff(host, noiseKey, 7, currentTime, activeNotes);
      noteOnPercussion(host, noiseKey, 80, currentTime, activeNotes, ym2151NoiseNoteForPeriod(state.noisePeriod));
    }
    return;
  }
...
export function syncYM2151ToneState(
  host: MidiConverter,
  channel: number,
  ...
  if (shouldSound && (!state.active || shouldRetrigger)) {
    if (state.active) noteOff(host, key, channel, currentTime, activeNotes);
    state.active = true;
...
export function syncYM2151NoiseState(
  host: MidiConverter,
  shouldRetrigger: boolean,
  currentTime: number,
  activeNotes: ActiveNoteMap
  ...
  if (shouldSound && (!state.isNoiseActive || shouldRetrigger)) {
    if (state.isNoiseActive) noteOff(host, noiseKey, 7, currentTime, activeNotes);
    ...
    noteOnPercussion(host,
      ...
      currentTime,
      activeNotes,
      ym2151NoiseNoteForPeriod(state.noisePeriod ?? 0)
    );
...
// HuC6280's $07.)
export function ym2151NoiseNoteForPeriod(nfrq: number): number {
