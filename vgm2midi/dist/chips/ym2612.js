...
exports.handleYM2612Write = handleYM2612Write;
exports.handleYM2612TimbreWrite = handleYM2612TimbreWrite;
exports.handleYM2612DACSeek = handleYM2612DACSeek;
exports.handleYM2612DACWrite = handleYM2612DACWrite;
exports.stopYM2612DACVoice = stopYM2612DACVoice;
exports.handleYM2612DirectDACWrite = handleYM2612DirectDACWrite;
exports.stopYM2612DirectDACVoice = stopYM2612DirectDACVoice;
const event_output_1 = require("../event-output");
const opn_shared_1 = require("./opn-shared");
function handleYM2612Write(host, cmd, currentTime, activeNotes, cmdIndex) {
    ...
    if ((0, opn_shared_1.handleOPNPanWrite)(host, 'ym2612', port, reg, data, currentTime))
        return;
    ...
    if (port === 0 && reg === 0x27) {
        ...
        (0, opn_shared_1.updateOPNCsmTimer)(host, 'YM2612', cmd.instance ?? 0, data, currentTime, activeNotes);
        return;
    }
    if (port === 0 && (reg === 0x24 || reg === 0x25)) {
        (0, opn_shared_1.updateOPNCsmTimerRegister)(host, 'YM2612', cmd.instance ?? 0, reg, data);
        return;
    }
    if (port === 0 && reg === 0x2B) {
    ...
    if (port === 0 && reg === 0x28) {
        ...
        if (channelIndex === 2 && host.isOPNCh3SpecialMode(ch3Context)) {
            (0, opn_shared_1.handleOPNCh3SpecialKeyWrite)(host, ch3Context, data, currentTime, activeNotes);
            return;
        }
        if (channelIndex !== -1) {
            ...
            if (keyOn && !state.active) {
                ...
                state.active = true;
                (0, event_output_1.noteOn)(host, key, channelIndex + 4, currentTime, activeNotes); // offset channel for MIDI
            }
            else if (!keyOn && state.active) {
                state.active = false;
                (0, event_output_1.noteOff)(host, key, channelIndex + 4, currentTime, activeNotes);
    ...
    if (channelOffset !== -1) {
        ...
        const isSplitUpdate = host.isOPNMultiByteFreqUpdate(cmdIndex, 'YM2612', port, otherReg, cmd.instance ?? 0);
        ...
        if (state.active && !isSplitUpdate && (state.frequency !== oldFreq || hadPendingUpdate)) {
            (0, event_output_1.updateNotePitch)(host, key, channelIndex + 4, currentTime, activeNotes);
        }
        return;
    }
    if (port === 0) {
        (0, opn_shared_1.handleOPNCh3SpecialFrequencyWrite)(host, ch3Context, reg, data, currentTime, activeNotes, cmdIndex);
    }
}
function handleYM2612TimbreWrite(host, port, reg, data, currentTime) {
    return (0, opn_shared_1.handleOPNTimbreWrite)(host, 'ym2612', port, reg, data, currentTime);
}
function handleYM2612DACSeek(host, cmd) {
    if (cmd.address === undefined)
        return;
    host.ym2612DACPendingAddress = cmd.address;
}
function handleYM2612DACWrite(host, currentTime) {
    ...
    if (!host.isYM2612DACEnabled)
        return;
    if (host.options.suppressYM2612Dac)
        return;
    stopYM2612DACVoice(host, currentTime);
    ...
    const descriptorId = (0, event_output_1.noteOnPCMPercussion)(host, trackKey, note, 100, currentTime, false, dataBlock);
    host.ym2612DACActiveVoice = { descriptorId, note };
}
function stopYM2612DACVoice(host, currentTime) {
...
function handleYM2612DirectDACWrite(host, currentTime) {
    if (!host.isYM2612DACEnabled)
        return;
    if (host.options.suppressYM2612Dac)
        return;
    ...
    if (host.ym2612DirectDACActiveVoice
        ...
        (0, event_output_1.noteOffPCMPercussion)(host, host.ym2612DirectDACActiveVoice.descriptorId, host.ym2612DirectDACActiveVoice.note, lastWriteTime);
        host.ym2612DirectDACActiveVoice = undefined;
    }
    if (!host.ym2612DirectDACActiveVoice) {
        const trackKey = 'ym2612dac_direct_stream';
        const note = (0, event_output_1.pcmNoteForSample)(host, trackKey);
        const descriptorId = (0, event_output_1.noteOnPCMPercussion)(host, trackKey, note, 100, currentTime);
...
function stopYM2612DirectDACVoice(host, currentTime) {
    const voice = host.ym2612DirectDACActiveVoice;
    if (!voice)
        return;
    const closeTime = host.ym2612DirectDACLastWriteTime ?? currentTime;
    (0, event_output_1.noteOffPCMPercussion)(host, voice.descriptorId, voice.note, closeTime);
    host.ym2612DirectDACActiveVoice = undefined;
